 ...
 */
@Component
public class YesterdayCalcuate0oclock  implements BaseJob{
  
	private Logger logger = LoggerFactory.getLogger(YesterdayCalcuate0oclock.class);
	 
	 
	private void before(){
		
        System.out.println("0点任务开始执行");
    }
		...
		try {
			...
			if (empList != null && empList.size() > 0 && empList.get(0) != null) {
				for (Map<String, Object> empMap : empList) {
					...
					if(attendGroup!=null){
						if (CommonConstant.固定班.getValue().equals(empMap.get("type"))) {
							...
							if (periodYestadayAttendList != null && periodYestadayAttendList.size() > 0
								...
								// 结算昨天的跨天考勤
								affectNum=acrossAttendCalcuate(periodYestadayAttendList, empId,tdSignInTime);
							}
							...
							if (attendYesterdayInfoList != null && attendYesterdayInfoList.size() > 0
								...
								// 结算昨天的跨天考勤
								affectNum=acrossAttendCalcuate(attendYesterdayInfoList, empId,tdSignInTime);
							}
						}
					}else{
						// 查询当前打卡员工考勤是否结算过
					...
					if(affectNum<0){
						logger.error("************0点定时任务结算昨天不跨天考勤失败*************");
					}
				}
			}
		} catch (Exception e) {
	...
	* @date 2019年5月30日上午11:26:51
	...
	public int acrossAttendCalcuate(List<Map<String, Object>> attendYesterdayInfoList,
		...
		if(attendYesterdayInfoList!=null && attendYesterdayInfoList.size()>0 && attendYesterdayInfoList.get(0)!=null){
			String attendType=(String)attendYesterdayInfoList.get(0).get("type");
